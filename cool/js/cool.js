...
window.onload = function() {
	...
	//获取音乐最大的ID
	var oDio = document.getElementById('dio');
	//获取右上角的声音关闭和开启的div
	var aNavDio = oDio.getElementsByTagName('div');
	//获取右上角的声音关闭和开启
	var aNavSpan = aNavDio[0].getElementsByTagName('span');
	//获取各种声音的地方
	...
	function dioAuto(a, b, c) {
		//那么右上角的声音的背景往左移到0px地方
		aNavSpan[0].style.left = a;
		//那么右上角的声音的音乐图标往左移到0px地方
		aNavSpan[1].style.left = a;
		//声音图标换成静音的样式
		aNavSpan[1].style.background = 'url(img/index/' + b + '.png)';
		//将所有声音开启，或者关闭
	...
	/*开机动画开始*/
	var oAin = document.getElementById('ain');
	//获取开机最大的div
	var aStart = document.getElementsByClassName('start')[0];
	var aBar1 = getElementsByClassName(oAin, 'div', 'bar1')[0];
	//进度条左边的圆
	var aBar2 = getElementsByClassName(oAin, 'div', 'bar2')[0];
	//进度条右边的圆
	var aAinJd = getElementsByClassName(oAin, 'span', 'jd')[0];

	var oHeader = document.getElementById('header');
	//获取导航位置
	var aHeDiv = oHeader.getElementsByTagName('div')[0];
	//获取导航的第一个div（用于进度条执行完毕后的动作）
	oAin.style.height = document.documentElement.clientHeight + 'px';
	/*进度条执行*/
	...
	//开启左边的定时器
	aBar1.timer = setInterval(function() {
	...
	function aBar2Auto() {
		...
		aBar3Auto(aBar2, 100, function() {
			//就停止掉定时器
			clearInterval(aBar2Auto);
			//开机画面隐藏
			oAin.style.display = 'none';
			//头顶导航往下移动
			aHeDiv.style.top = '0px';
		});
	}
	/*需要的秒数*/
	//了解一个人部分需要的秒数
	var aAinCon = getElementsByClassName(oAin, 'span', 'con1')[0];
	//最先为0秒
	aAinCon.num = 0;
	...
	function aBar3Auto(a, b, fn) {
		...
		if (a.num < 0) { //如果圆旋转度数小于或者等于0
			...
			fn && fn();
		} //进度条不断的赋值给innerHTML，并且因为是小数，所以取值为整数
		aAinJd.innerHTML = parseInt(aAinJd.num);
		//兼容webkit度数不断的赋值
		a.style.WebkitTransform = 'rotate(' + a.num + 'deg)';
		//兼容moz度数不断的赋值
	...
	var _this1 = 1;
	//盒子的高度 = 客户端的高度
	oColl.style.height = document.documentElement.clientHeight + 'px';
	oColl1.style.height = document.documentElement.clientHeight + 'px';
	oWork.style.height = document.documentElement.clientHeight + 'px';
	oXxbg.style.height = document.documentElement.clientHeight + 'px';
	oWork1.num = 0;
	oColl1.num = 0;
	oXxbg1.num = 0;
	...
	function cs() {
		setTimeout(function() {
			aCollUl.style.left = 0 + "px";
			oXian.style.left = 0 + "px";
			for (var i = 0; i < oBg1.length; i++) {
				oXian.style.display = 'block';
				oCollLi[i].className = 'coll_li';
				bg1Auto(oBg1[i], 1, oBg3Ul[i], 'block');
	...
	aShade.style.height = document.documentElement.clientHeight + 'px';
	...
	var aTimeArr = ['2017.4.17-2018.4.1', '', '', '', '', '', '', ''] //记录时间
	var aTimeArr1 = ['工作职位：web前端', '工作职位：敬请期待', '工作职位：敬请期待', '工作职位：敬请期待', '工作职位：敬请期待', '工作职位：敬请期待', '工作职位：敬请期待', '工作职位：敬请期待'] //工作时间
	var aTimeArr2 = [
		'主要负责对产品的开发，psd还原',
		'敬请期待',
		'敬请期待',
